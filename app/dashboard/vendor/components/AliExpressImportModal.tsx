...
const AliExpressImportModal: React.FC<AliExpressImportModalProps> = ({ isOpen, onClose }) => {
    ...
    const handleSearch = async () => {
        ...
        setLoading(true);
        try {
            const response = await fetch(`${backendUrl}/api/v1/import/search/aliexpress`, {
                method: 'POST',
                    ...
                    {searchResults.length > 0 ? (
                        searchResults.map((product, index) => (
                            <div key={product.id || index} className="border p-4 rounded">
                                <img
                                    src={product?.image ? `https:${product.image}` : "https://via.placeholder.com/150"}
